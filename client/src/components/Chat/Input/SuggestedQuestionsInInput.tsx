...
const EMBEDDED_SUGGESTED_QUESTIONS = [
  'Summarize the key details for this project.',
  'Show me prior-year tax returns.',
  'Show me current tax year PBC docs.',
  'Has the client submitted the tax questionnaire?',
  'Did the client report major life changes?',
  'Are there large year-over-year income variances?',
  'Is the return ready for preparer review?',
  'Are any prior-year W-2s missing this year?',
  'Are there any new 1099-INT payers this year?',
] as const;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={cn(
          'absolute bottom-2 right-2 z-10 flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center',
          ...
          'shadow-[0_0_12px_rgba(251,191,36,0.25)] motion-safe:animate-pulse',
        )}
        aria-expanded={isOpen}
        aria-haspopup="true"
        aria-label="Suggested questions"
      >
        <Lightbulb className="h-5 w-5 text-amber-400/90" />
      </button>
      ...
      <div
        ...
        className={cn(
          'absolute bottom-14 right-2 z-20 grid overflow-hidden transition-all duration-200 ease-out outline-none',
          isOpen ? 'max-h-[280px] opacity-100' : 'max-h-0 opacity-0',
        )}
        style={{ width: '18rem', maxWidth: 'calc(100vw - 2rem)' }}
      >
        <div className="flex max-h-[264px] flex-col gap-1.5 overflow-y-auto rounded-lg border border-border-light bg-surface-primary p-2 shadow-lg">
